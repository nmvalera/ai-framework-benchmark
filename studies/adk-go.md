...
- **Biggest gap for our use case**: **No tenant-scoped filtering primitives, no per-tenant budget caps, no allow/deny tool sandbox, no eval framework in Go**. Eval endpoints are stubbed as `controllers.Unimplemented` (`server/adkrest/internal/routers/eval.go:30-46`). The eval story is **Python-only** (the matrix's 🔴 cells are correct — verified). Tool sandboxing is **not provided — BYO**: tools execute as raw Go code with no allow/deny list (the matrix 🔴 is correct). Per-tenant budget/USD cap is **not provided — BYO**: no rate-limiter, no cost computation, only token counts in `genai.GenerateContentResponseUsageMetadata` (`session/database/storage_session.go:89`).
...
**Important**: every box inside the "Go binary" runs in the **same OS process**. There is no subprocess, no sidecar, no JSON-RPC bridge. This is the simplest deployment shape of any of the 11 stacks benchmarked.
...
- `google.golang.org/genai v1.57.0` (the GenAI Go SDK — both Vertex AI and Gemini API paths)
- `github.com/gorilla/mux v1.8.1` (REST routing)
- `gorm.io/gorm v1.31.0` (database session backend) + driver of your choice (`glebarez/sqlite` ships for tests; the user picks postgres/mysql/spanner)
- `go.opentelemetry.io/otel v1.43.0` + log/trace exporters
- `github.com/modelcontextprotocol/go-sdk v1.4.1` (MCP toolset)
- `github.com/a2aproject/a2a-go v0.3.15` + v2 (A2A protocol server)
- `cloud.google.com/go/storage v1.56.1` (GCS artifact backend)
- `cloud.google.com/go/aiplatform v1.121.0` (Vertex AI session backend)
- `golang.org/x/sync v0.20.0` (errgroup for parallel agents)

No bundled binaries. No native libs. No optional Postgres unless you use `session/database`. No optional Redis. **No mandatory Vertex** — you can run with `gemini` API key only.
...
| Eval platform | **N/A in adk-go** | Eval is Python-only (see Q17). |
...
- Official docs: `https://google.github.io/adk-docs/` (referenced from README line 18).
...
- **Quickstart**: https://google.github.io/adk-docs/get-started/ (general docs root → Get Started)
...
- **GitHub issues tracker**: https://github.com/google/adk-go/issues — relevant issue categories to watch: anything tagged `multi-tenant`, `scaling`, `eval`. **No open issue equivalent to Claude SDK's #333 (startup latency)** was found because adk-go cold-starts in <1s.
...
The matrix mentioned `adk.dev` — that is a long-running agent alias / redirect for the docs; the canonical URL is `google.github.io/adk-docs/`.
...
Two implementations: `memory.InMemoryService()` and `memory/vertexai/` (RAG-backed). See Q15.
...
This is the **same gap as 8 of the 11 stacks benchmarked** — no agent framework solves this in a useful way; it's the application's job.
...
func main() {
    ...
    topicSearchTool, _ := functiontool.New(
        functiontool.Config{Name: "topicSearch", Description: "Search long-running agent topics."},
        topicSearch,
    )
...
### 5.7 Architectural diagram of where hooks fire across the loop
...
func main() {
    ...
    instructionProvider := func(ctx agent.ReadonlyContext) (string, error) {
        ...
        return fmt.Sprintf("You are a long-running agent assistant. tenant=%v, locale=%v, today=2026-05-16. " +
...
- Forced tool args (Q4.4 + Q5.3) and `AfterToolCallback` summarization both work via in-place map mutation.
...
Sample frames (already shown in 1.12).
...
curl -N -X POST http://localhost:8080/run_sse \
  ...
  -d '{
    ...
    "userId": "u-123",
    "sessionId": "sess-1",
    "newMessage": {"role":"user","parts":[{"text":"find long-running agent topics for surfers"}]},
    "streaming": true,
...
#
# data: {"id":"02...","author":"long-running agent_agent","content":{"role":"model","parts":[{"functionCall":{"id":"call_1","name":"topicSearch","args":{"query":"surfers"}}}]},"actions":{}}
#
# data: {"id":"03...","author":"long-running agent_agent","content":{"role":"user","parts":[{"functionResponse":{"id":"call_1","name":"topicSearch","response":{"results":["surfing","beach"]}}}]},"actions":{}}
#
# data: {"id":"04...","author":"long-running agent_agent","content":{"role":"model","parts":[{"text":"Top topics: surfing, beach"}]},"actions":{}}
...
func main() {
    ...
    youngMom, _ := llmagent.New(llmagent.Config{
        ...
        Description: "Long-running agent persona: young mom, 25-35, suburban.",
        Model: model,
    ...
    techBro, _ := llmagent.New(llmagent.Config{
        ...
        Description: "Long-running agent persona: tech bro, 22-38, urban.",
        Model: model,
    ...
    retiree, _ := llmagent.New(llmagent.Config{
        ...
        Description: "Long-running agent persona: retiree, 65+, suburban/rural.",
        Model: model,
    ...
    parentLLM, _ := llmagent.New(llmagent.Config{
        Name: "long-running agent_supervisor",
        Model: model,
...
// description: Turn a long-running agent brief into a targeted audience definition. Use when the user supplies a free-text brief mentioning age, geo, interests, or behavior.
// allowed-tools:
//   - iabSearch
//   - topicSearch
//   - audienceCreate
...
The matrix flag of `YAML-config + skill source` is accurate for **versioning** if you check your skills into git and the YAML config (`internal/configurable/`) references config files by path — that's "versioning via git". Otherwise BYO.
...
`controllers.Unimplemented` returns HTTP 501. **This is the matrix's 🔴 verified.** If you adopt adk-go and need eval, you'd run a sister `adk-python` process for eval workloads or write your own harness.
