...
> **Commit analysed**: 6a95bb5c4ffe0bb4e9969eefa8ccc38ba19193b6
> **Branch**: main
> **Framework path**: frameworks/strands-agents-sdk-typescript
> **Analysed on**: 2026-05-16
...
- **Ecosystem**: TypeScript (Node.js ≥ 20). Also compiles to WASM, consumed from a sibling Python SDK (`strands-py`) — but the primary ecosystem is TypeScript.
...
**TypeScript** (primary). Compiles to WASM (`strands-wasm/`) and is consumed from a sibling Python SDK (`strands-py`) via a WIT (WebAssembly Interface Type) contract; that is a guest-runtime arrangement, not a separate language implementation.
...
- **Usage pattern**: library (you import into your own Node host). No hosted platform or CLI runtime.
...
- **External binaries / subprocesses**: none. No bundled CLI, no `claude-code`-style external runtime.
- **Vendor services**: only the LLM provider you choose (Bedrock by default; otherwise Anthropic / OpenAI / Google / any AI-SDK provider).

### 1.3 Recommended deployment topology

Not prescribed in the repo. The README and examples treat the SDK as a normal Node library. A host typically would put `new Agent(...)` instances behind an HTTP layer (Express/Hono/Fastify) — there's no shipped "agent runtime daemon". Practical patterns: one-process-many-agents (instantiate per request) or container-per-tenant if you want strong isolation.

### 1.4 Cold-start cost & instance footprint

Not measured; expected to be light because no subprocess starts. Lazy-loaded peer-deps mean tree-shaken bundles can stay small. The browser bundle check in CI suggests modest baseline footprint. No equivalent of Claude Agent SDK's 20–30 s subprocess spawn.
...
No `CHANGELOG.md` in-repo; release notes are inferred from GitHub Releases and `git log`. Recent themes (last 30 commits before the analysed head):
...
Not built-in. See Q17 (Memory & Knowledge).
...
- **Mutate tool input before dispatch**: yes — `BeforeToolCallEvent.toolUse.input = …` (see Q6.4).
...
**Library-only by default**. The only shipped HTTP surface is the optional `A2AExpressServer` (`strands-ts/src/a2a/express-server.ts:37`), which exposes an `InvokableAgent` as an **Agent-to-Agent (A2A) protocol JSON-RPC endpoint** at `POST /` plus an agent-card discovery endpoint at `GET /.well-known/agent-card.json`. This is not a multi-tenant chat server. The exposing package is `@strands-agents/sdk/a2a/express`.

### 8.2 HTTP streaming transport

A2A uses the upstream `@a2a-js/sdk` package's transports — JSON-RPC with optional streaming. For your own host, you choose: SSE / WebSocket / HTTP-stream / whatever. The `agent.stream()` generator yields plain JS objects you serialize as you wish. **No proprietary stream format is enforced** by the SDK.
...
There is no canonical wire format the SDK enforces. The frames in Q3.6 are the in-process shape; you serialize them however your client expects. `Event.toJSON()` methods on every hook event class (`hooks/events.ts:129`, `:160`, `:201`, ...) define how events serialize for wire transport — they intentionally drop the `agent` and `invocationState` references.
...
Three primitives at the SDK layer (the HTTP layer is BYO):
...
2. `InvokeOptions.cancelSignal: AbortSignal` (`strands-ts/src/types/agent.ts:120`) composes an external signal with the internal one. **This is the API-friendly path** — bind to `req.signal` from your HTTP framework so a client closing the stream tears down the run.
3. Inside a tool, `context.agent.cancelSignal.aborted` for cooperative cancellation.

Recommended HTTP shape: `DELETE /runs/:runId` → call `agent.cancel()` on the corresponding agent instance.
...
The HTTP wiring (an endpoint that accepts an approval verdict and re-invokes the agent with an `InterruptResponseContent` block) is BYO.
...
For **MCP**: any MCP server reachable via stdio / HTTP / SSE / streaming-HTTP transport (Q14).
...
Honest verdict: Resource Manager is the **largest gap** in Strands TS for our multi-tenant skill library use case. Q10 (skills format) is fine; Q11 (skill platform) is BYO.
...
Not provided as a first-class subsystem. Build a retrieval tool: a `tool({ name: 'retrieve', callback: async ({ query }) => doVectorSearch(query) })` is the standard pattern. The `ContextOffloaderPlugin` (Q7.6) is the closest to a built-in retrieval pattern — it offloads tool results to disk/blob and exposes a `retrieve_offloaded_content` tool the model can use to grep/page through them.
