...
> **Commit analysed**: 57ebeaa4e18989909013ebd58351b3ef6a5586e0
> **Branch**: main
...
### 0.2 Ecosystem
**Rust** — primary implementation language (edition 2024 in the workspace; README says "latest stable").

Second language: **Python**, via the `bindings/python/` crate (`autoagents-py`) which uses PyO3 + maturin to expose the Rust core to Python users. The Python surface is a thin wrapper, not the canonical API.
...
- README declares the project "**production-grade**", but the 0.3.x version, lack of session/HTTP primitives, and absence of changelog files (`CHANGELOG.md`/`HISTORY.md`/`RELEASES.md` are all absent — verified) suggest pre-1.0 maturity. APIs are not formally marked experimental/stable but the major-version-zero signals breaking changes are still possible.

### 0.5 Adoption & community signal
Captured via README/repo on 2026-05-19 (live GitHub numbers cannot be fetched from this environment):
...
- The most recent visible commit is "[MAINT]: Fix CI Issues (#228)" — issue/PR numbers up to ~228 visible.
- Cannot quantify exact stars/forks/contributor counts from this offline study; readers should check https://github.com/liquidos-ai/AutoAgents directly.
...
- Language: **English only** (READMEs are translated but docs site is English).
...
`crates/autoagents-core/src/agent/prebuilt/executor/react.rs:264` then drives a loop of `engine.run_turn(...)` calls up to `max_turns` (default 10). The LLM HTTP call happens in `TurnEngine::get_llm_response` (`turn_engine.rs:511`), invoked from `run_turn` (`turn_engine.rs:178`). All this runs as ordinary `async fn` on the calling task — no subprocess, no daemon, no vendor cloud.
...
- **External binaries**: only MCP servers (subprocess via stdio) if you use them — otherwise zero.
...
**Not provided — BYO.** No long-term memory abstraction. (See Q17 — same answer.)
...
**Bottom line for Q7**: hooks in AutoAgents are **observe-and-abort**, not mutate-and-decorate. For the three patterns in the prompt — system-prompt injection at session start, forced tool args, post-tool result summarization — none are first-party.
...
**Not provided — BYO HTTP layer.** `run_stream` returns a `BoxRuntimeStream<Result<<T>::Output, Error>>` in-process; you'd serialize each frame to SSE / WebSocket in your own HTTP handler.

### 8.3 HTTP endpoints that start an agent run
**Not provided — BYO HTTP layer.**

### 8.4 Live agentic event stream format
**In-process only.** `Environment::subscribe_events(runtime_id)` yields a `BoxEventStream<Event>` of the protocol `Event` enum (which is `Serialize/Deserialize`, so you can JSON-encode it to your wire format). See Q3.6 for sample frames.
...
**Partial — abort-only.** `AgentHooks::on_tool_call` returning `HookOutcome::Abort` skips the call (`tool_processor.rs:60`). But there is no "pause and wait for external approval" state — the run cannot be persisted and resumed. Surfacing this to an HTTP client is BYO.
...
```rust
// Not provided — BYO HTTP layer. Sketch only.
...
There is **no linkage back to a parent `tool_use_id`** because sub-agents are not modelled as tool calls. The matching key is `(sub_id, actor_name)`, not a parent tool-call id.
...
**Strong by default.** Each agent has its own `BaseAgent` with its own `memory`, `tools`, and `Context`. Parent's memory is not shared. You can pass *data* via the `Task::prompt` you publish, but the parent's chat history is not. Isolation is enforced by `BaseAgent` ownership of `Arc<Mutex<Box<dyn MemoryProvider>>>` and the absence of any shared mutable state at the framework level.
...
This is a local-filesystem TOML loader for MCP servers. It is *not* a general resource manager.
...
### 16.4 BYO pattern
Wire your own frontend to your own HTTP layer (Q8.1) and parse the `Event` JSON into UI state. The events are `Serialize/Deserialize`, so the conversion to a wire format (SSE / WebSocket frames) is straightforward.
